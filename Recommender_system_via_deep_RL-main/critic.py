...
class CriticNetwork(tf.keras.Model):
    def __init__(self, embedding_dim,hidden_dim):
        ...
        self.inputs = tf.keras.layers.InputLayer(input_shape=(embedding_dim,embedding_dim, 3*embedding_dim))
...
class Critic(object):
    ...
    def build_networks(self):
        self.network([np.zeros((1,self.embedding_dim)),np.zeros((1,self.embedding_dim)), np.zeros((1,3*self.embedding_dim))])
        self.target_network([np.zeros((1,self.embedding_dim)),np.zeros((1,self.embedding_dim)), np.zeros((1,3*self.embedding_dim))])
